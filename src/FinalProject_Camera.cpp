...
using namespace std;
//mHorizonDump(HD_FILE, std::ios::out)
/* MAIN PROGRAM */

void mainLoop(std::string const& detectorType, std::string const& descriptorType, ofstream& outputFile, bool bVis = false)
{
    ...
    //cout << "|Ref Velocity|TTC Lidar|Computation Method|"<< '\n';
    //cout << "|----|----|----|"<< '\n';
    //outputFile << "|TTC Camera|Detector/Descriptor pair|"<< '\n';
    //outputFile << "|----|----|"<< '\n';
    outputFile << "TTC Camera,Detector/Descriptor pair"<< '\n';
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        //string detectorType = "ORB";//"SHITOMASI";
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            //cout << "#7 : MATCH KEYPOINT DESCRIPTORS done with "<< matches.size() << " matches." << endl;
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    //cout <<  "Matching BoxIds "<< prevBB->boxID << ", "<< currBB->boxID << '\n';
                    //// EOF STUDENT ASSIGNMENT
                    ...
                    // If enabled you can see the match points
                    if (bVis)
                    {
                        std::string windowName = "Matched BB";
                        cv::namedWindow(windowName, 5);
                        cv::Mat visImage;
                        cv::drawMatches((dataBuffer.end() - 2)->cameraImg, (dataBuffer.end() - 2)->keypoints, 
                            (dataBuffer.end() - 1)->cameraImg, (dataBuffer.end() - 1)->keypoints,currBB->kptMatches,visImage, 
                        cv::Scalar::all(-1), cv::Scalar::all(-1));
                        cv::imshow(windowName, visImage);
                        cv::waitKey(0);
                    }
                    computeTTCCamera((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                    outputFile << ttcCamera << ","<<detectorType << "/"<< descriptorType << "\n";
                    //// EOF STUDENT ASSIGNMENT
...
int main(int argc, const char *argv[])
{
    ...
    for (auto const &detector : KPT_DETECTORS)
    {
        ...
        for (auto const &descriptor : KPT_DESCRIPTORS)
        {
            ...
            mainLoop(detector, descriptor, performanceDump);
        }
    }
